...
PaperBinder is intentionally narrow: a multi-tenant SaaS demonstration that prioritizes tenant isolation, explicit boundaries, and reviewable engineering decisions over feature breadth. The current PaperBinder release is `v1.1.1`, fully implemented, validated, and documented; `v1.1.1` has merged to `main`, the `v1.1.1` tag's `release.yml` run has passed (GHCR images and a draft GitHub Release published), and `v1.1.1` is deployed and smoke-verified on Test. Prod deployment, production smoke validation, and publishing the release remain the outstanding owner-controlled release actions, tracked in `docs/95-delivery/release-checklist.md`.
