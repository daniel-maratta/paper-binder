...
PaperBinder is intentionally narrow: a multi-tenant SaaS demonstration that prioritizes tenant isolation, explicit boundaries, and reviewable engineering decisions over feature breadth. The current published stable PaperBinder release is `v1.1.2`: merged to `main`, tagged, deployed and smoke-verified on Test, with its GitHub Release published. The active `v1.1.3` hotfix branch stages a narrow mobile public-header correction; see `docs/95-delivery/release-checklist.md` for the full evidence trail.
