...
| 11 | **Private-boundary scan.** Grep for `magenta`, `mappcore`, `MagentaAI`, `MappCore` (case-insensitive) across `*.ts`, `*.tsx`, `*.md`, `*.json`, `*.ps1`, `*.yml`: only match is `AGENTS.local.md`, which is local-only and excluded from commits. No private-boundary leakage in public artifacts. | Pass |
