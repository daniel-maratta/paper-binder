...
- `Release`: a stable shipped cut on the `V1` line. The initial reviewer-ready cut used prose `V1` with tag `v1.0.0`; the current PaperBinder release is `v1.1.1`, superseding the prior published stable tag `v1.1.0`. `v1.1.1` has merged to `main`, the `v1.1.1` tag's `release.yml` run has passed, and `v1.1.1` is deployed on both Test and Prod; publishing the draft GitHub Release is the remaining owner-controlled release action.
