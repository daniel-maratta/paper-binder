...
- The `v1.1.1` release-close-out task (`T-0055`) has recorded main as taggable; `v1.1.1` has merged to `main`, the `v1.1.1` tag's `release.yml` run has passed (GHCR images published, draft GitHub Release created), and `v1.1.1` is deployed and smoke-verified on both Test and Prod. Publishing the draft GitHub Release remains an owner-controlled action per the sequence below.
- Changelog cut shape:
...
3. Update release artifacts.
   ...
   - Keep active release-candidate notes under `Unreleased` until the final release close-out task records that `main` is taggable.
