...
- Update `docs/archive/v1/checkpoints/checkpoint-status.md` to mark the checkpoint `active` when execution starts.
...
- Update `docs/archive/v1/checkpoints/checkpoint-status.md` to mark the checkpoint `done`, advance the snapshot, and record any checkpoint-level follow-ups.
...
- If work reveals a follow-up that affects checkpoint readiness, sequencing, or closure, record the detailed item in the taskboard and add a short checkpoint-level note to `docs/archive/v1/checkpoints/checkpoint-status.md`.
