...
- Mark the checkpoint `active` in `docs/archive/v1/checkpoints/checkpoint-status.md` when the checkpoint's first task is pulled into execution.
...
- If the discovered work affects checkpoint sequencing, readiness, or merge closure, add a short note to `docs/archive/v1/checkpoints/checkpoint-status.md` after recording the durable detail in the taskboard.
