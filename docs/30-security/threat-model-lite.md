...
- Analytics data leakage
  - Mitigations: GoatCounter usage analytics are sent from PaperBinder-owned code as constrained direct `/count` image requests, not by executing analytics-provider JavaScript. Pageviews use explicit public routes or tenant route templates, public events use approved low-cardinality `pb_event_public_` names, internal referrers are suppressed, and external referrer queries/fragments are stripped. Analytics payloads must not include tenant slugs, user identifiers, binder identifiers, document identifiers, emails, user-provided names, document titles, or document content.
