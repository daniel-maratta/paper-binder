...
- `v1.1.1` has merged to `main`, the `v1.1.1` tag's `release.yml` run has passed (GHCR images published, draft GitHub Release created), and `v1.1.1` is deployed and smoke-verified on Test; Prod deployment remains the outstanding owner-controlled release action, tracked in `docs/95-delivery/release-checklist.md`
