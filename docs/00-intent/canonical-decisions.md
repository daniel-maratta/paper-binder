...
Release boundary:
- The shipped release line is prose `V1`; the current PaperBinder release is `v1.1.3`, fully implemented, validated, released, and documented, with `v1.1.2` as the prior published stable tag
- `v1.1.3` is the current published stable tag: merged to `main`, tagged, deployed and smoke-verified on Test and Prod, with its GitHub Release published, per `docs/95-delivery/release-checklist.md`
