...
- CP15 adds trusted impersonation markers to that same parent-domain auth cookie so actor identity remains the authenticated principal while effective identity can change within the tenant.
...
- Effective impersonated membership is still tenant-local by construction because target lookup happens only inside the host-resolved tenant.
- Expired impersonation cookies are closed on the next tenant-host request, which records `ImpersonationEnded` before clearing the stale session.
...
- CP15 widens that request-scoped context to carry both actor and effective identity plus an optional impersonation session id.
