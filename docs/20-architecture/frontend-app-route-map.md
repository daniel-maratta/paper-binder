...
| `/start-demo` | Dedicated demo-entry flow for provision, login, and handoff | `POST /api/provision`; `POST /api/auth/login` | Anonymous allowed | Owns challenge-protected provisioning, direct-login affordances, and the shown-once generated-credentials handoff state. Successful provision or login still navigates only through the server-provided `redirectUrl`. |
...
| `/articles/building-paperbinder-production-shaped-saas-demo` | Hosted flagship article for the PaperBinder build and architecture story | none | Anonymous allowed | Linked from the About page. This is static public content and does not establish tenant context or call APIs. The API serves route-specific initial HTML metadata for crawlers/unfurlers, and the SPA renders the Markdown web representation of the accepted RC2 article body with PaperBinder publication chrome. |
