...
Root-host flows are live in CP13, and tenant-host routes are live in CP14.
...
| --- | --- | --- | --- | --- |
| `/app` | Tenant home dashboard + lease visibility | `GET /api/tenant/lease`, `GET /api/binders` | Authenticated tenant member | Lease banner is always visible in the tenant shell; dashboard shows reviewer-useful recent binders without a new backend aggregator endpoint. |
...
| `/app/users` | Tenant user management | `GET /api/tenant/users`, `POST /api/tenant/users`, `POST /api/tenant/users/{userId}/role` | `TenantAdmin` | Admin-only route; non-admin must receive safe forbidden behavior inside the current tenant shell. |
