...
internal static class TenantResolutionIntegrationTestHost
{
    ...
    public static async Task ExpireTenantAsync(
        ...
        SeededTenant tenant,
        DateTimeOffset? expiresAtUtc = null,
        DateTimeOffset? lastAuthenticatedActivityAtUtc = null)
    {
        var connectionFactory = host.Application.Services.GetRequiredService<ISqlConnectionFactory>();
        await using var connection = await connectionFactory.OpenConnectionAsync();
        await connection.ExecuteAsync(
            ...
            set expires_at_utc = @ExpiresAtUtc,
                last_authenticated_activity_at_utc = @LastAuthenticatedActivityAtUtc
            where id = @TenantId;
            """,
            new
            {
                ExpiresAtUtc = expiresAtUtc ?? DateTimeOffset.UtcNow.AddMinutes(-1),
                LastAuthenticatedActivityAtUtc = lastAuthenticatedActivityAtUtc,
                TenantId = tenant.Id
            });
    }
